...
func (th *TaskHandler) ApplyTasks(ctx context.Context, draft repository.PackageDraft, repositoryObj *configapi.Repository, obj *api.PackageRevision, packageConfig *builtins.PackageConfig) error {
...
func (th *TaskHandler) mapTaskToMutation(ctx context.Context, obj *api.PackageRevision, task *api.Task, isDeployment bool, packageConfig *builtins.PackageConfig) (mutation, error) {
	switch task.Type {
...
func (th *TaskHandler) conditionalAddRender(subject client.Object, mutations []mutation) []mutation {
...
func (th *TaskHandler) DoPRMutations(ctx context.Context, namespace string, repoPR repository.PackageRevision, oldObj *api.PackageRevision, newObj *api.PackageRevision, draft repository.PackageDraft) error {
	ctx, span := tracer.Start(ctx, "TaskHandler::DoPRMutations", trace.WithAttributes())
	defer span.End()
...
func (th *TaskHandler) DoPRResourceMutations(ctx context.Context, pr2Update repository.PackageRevision, draft repository.PackageDraft, oldRes, newRes *api.PackageRevisionResources) (*api.RenderStatus, error) {
	ctx, span := tracer.Start(ctx, "TaskHandler::DoPRResourceMutations", trace.WithAttributes())
	defer span.End()
