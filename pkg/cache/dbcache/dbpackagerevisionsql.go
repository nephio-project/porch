...
func pkgRevReadFromDB(ctx context.Context, prk repository.PackageRevisionKey, readResources bool) (*dbPackageRevision, error) {
	...
	sqlStatement := `
		SELECT
			...
			package_revisions.exists_on_ext,
			package_revisions.deleting_on_ext,
...
func pkgRevWriteToDB(ctx context.Context, pr *dbPackageRevision) error {
	...
	sqlStatement := `
        INSERT INTO package_revisions (k8s_name_space, k8s_name, package_k8s_name, revision, meta, spec, updated, updatedby, lifecycle, exists_on_ext, deleting_on_ext, ext_pr_id, tasks)
	...
	if _, err := GetDB().db.Exec(
		...
		prk.PKey().K8SName(), prk.Revision, valueAsJSON(pr.meta), valueAsJSON(pr.spec), pr.updated, pr.updatedBy, pr.lifecycle, pr.existsOnExt, pr.deletingOnExt, valueAsJSON(pr.extPRID), valueAsJSON(pr.tasks)); err == nil {
...
func pkgRevUpdateDB(ctx context.Context, pr *dbPackageRevision, updateResources bool) error {
	...
	result, err := GetDB().db.Exec(
		sqlStatement,
		prk.K8SNS(), prk.K8SName(),
		prk.PKey().K8SName(), prk.Revision, valueAsJSON(pr.meta), valueAsJSON(pr.spec), pr.updated, pr.updatedBy, pr.lifecycle, pr.existsOnExt, pr.deletingOnExt, valueAsJSON(pr.extPRID), valueAsJSON(pr.tasks))
