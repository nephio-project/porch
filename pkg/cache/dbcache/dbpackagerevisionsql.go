...
func pkgRevReadFromDB(ctx context.Context, prk repository.PackageRevisionKey, readResources bool) (*dbPackageRevision, error) {
	...
	sqlStatement := `
		SELECT
			...
			package_revisions.ext_repo_state,
...
func pkgRevScanRowsFromDB(ctx context.Context, rows *sql.Rows) ([]*dbPackageRevision, error) {
	...
	for rows.Next() {
		...
		var pkgK8SName, prK8SName, metaAsJSON, specAsJSON, extRepoState, extPRID, tasks string
		...
		pkgRev.extRepoState = toDeploymentState(extRepoState)
...
func pkgRevWriteToDB(ctx context.Context, pr *dbPackageRevision) error {
	...
	sqlStatement := `
        INSERT INTO package_revisions (k8s_name_space, k8s_name, package_k8s_name, revision, meta, spec, updated, updatedby, lifecycle, ext_repo_state, ext_pr_id, tasks)
	...
	if _, err := GetDB().db.Exec(
		...
		prk.PKey().K8SName(), prk.Revision, valueAsJSON(pr.meta), valueAsJSON(pr.spec), pr.updated, pr.updatedBy, pr.lifecycle, pr.extRepoState.String(), valueAsJSON(pr.extPRID), valueAsJSON(pr.tasks)); err == nil {
...
func pkgRevUpdateDB(ctx context.Context, pr *dbPackageRevision, updateResources bool) error {
	...
	sqlStatement := `
        UPDATE package_revisions SET package_k8s_name=$3, revision=$4, meta=$5, spec=$6, updated=$7, updatedby=$8, lifecycle=$9, ext_repo_state=$10, ext_pr_id=$11, tasks=$12
	...
	result, err := GetDB().db.Exec(
		sqlStatement,
		prk.K8SNS(), prk.K8SName(),
		prk.PKey().K8SName(), prk.Revision, valueAsJSON(pr.meta), valueAsJSON(pr.spec), pr.updated, pr.updatedBy, pr.lifecycle, pr.extRepoState.String(), valueAsJSON(pr.extPRID), valueAsJSON(pr.tasks))
