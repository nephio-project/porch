...
func pkgReadFromDB(ctx context.Context, pk repository.PackageKey) (*dbPackage, error) {
	...
	rows, err := GetDB().db.QueryContext(ctx, sqlStatement, pk.K8SNS(), pk.K8SName())
	if err != nil {
...
func pkgListPkgsFromDB(ctx context.Context, filter repository.ListPackageFilter) ([]*dbPackage, error) {
	...
	rows, err := GetDB().db.QueryContext(ctx, sqlStatement)
	if err != nil {
...
func pkgReadPkgsFromDB(ctx context.Context, rk repository.RepositoryKey) ([]*dbPackage, error) {
	...
	rows, err := GetDB().db.QueryContext(ctx, sqlStatement, rk.Namespace, rk.Name)
	if err != nil {
		return nil, err
	}
...
func pkgWriteToDB(ctx context.Context, p *dbPackage) error {
	...
	pk := p.Key()
	if _, err := GetDB().db.ExecContext(ctx, sqlStatement,
		pk.K8SNS(), pk.K8SName(),
...
func pkgUpdateDB(ctx context.Context, p *dbPackage) error {
	...
	pk := p.Key()
	result, err := GetDB().db.ExecContext(ctx,
		sqlStatement,
		pk.K8SNS(), pk.K8SName(),
...
func pkgDeleteFromDB(ctx context.Context, pk repository.PackageKey) error {
	...
	if _, err := GetDB().db.ExecContext(ctx, sqlStatement, pk.K8SNS(), pk.K8SName()); err == nil {
