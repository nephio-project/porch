...
export skip_variant_sets=${SKIP_VARIANT_SETS:-false}
export skip_variants=${SKIP_VARIANTS:-false}
...
function resetTestWorkload() {
    ...
    if [[ $skip_variant_sets != "true" ]]; then
        ...
        kubectl_dbcache wait packagevariantsets --all --all-namespaces --for 'condition=Ready' --timeout 5m
    fi
    ...
    if [[ $skip_variant_sets != "true" || $skip_variants != "true" ]]; then
        ...
        kubectl_dbcache wait packagevariants --all --all-namespaces --for 'condition=Ready' --timeout 10m
    fi
