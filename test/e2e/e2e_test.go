...
func (t *PorchSuite) TestKptfileUpstreamFormatConsistency() {
	// Register upstream repository
	t.RegisterGitRepositoryF(testBlueprintsRepo, "test-blueprints", "")

	// Create two package revisions - one simulating get command, one simulating clone command
	upstreamRef := &porchapi.PackageRevisionRef{
		Name: "basens-v1",
	}

	getPR := &porchapi.PackageRevision{
		TypeMeta: metav1.TypeMeta{
			Kind:       "PackageRevision",
			APIVersion: porchapi.SchemeGroupVersion.Identifier(),
		},
		...
		Spec: porchapi.PackageRevisionSpec{
			PackageName:    "basens",
			WorkspaceName:  "v1",
			RepositoryName: "test-blueprints",
			Tasks: []porchapi.Task{
				{
					Type: porchapi.TaskTypeClone,
					Clone: &porchapi.PackageCloneTaskSpec{
	...
	// Create both package revisions
	t.CreateF(getPR)
	t.CreateF(clonePR)

	// Get resources for both packages
	var getResources porchapi.PackageRevisionResources
	t.GetF(client.ObjectKey{
		Namespace: t.Namespace,
		Name:      getPR.Name,
	}, &getResources)

	var cloneResources porchapi.PackageRevisionResources
	t.GetF(client.ObjectKey{
		Namespace: t.Namespace,
		Name:      clonePR.Name,
	}, &cloneResources)
	...
	// Parse Kptfiles
	getKptfile := t.ParseKptfileF(&getResources)
	cloneKptfile := t.ParseKptfileF(&cloneResources)

	// Compare upstream sections
	if getKptfile.Upstream == nil || cloneKptfile.Upstream == nil {
		t.Fatalf("Expected both Kptfiles to have upstream sections")
	}

	// Compare upstream lock sections
	if getKptfile.UpstreamLock == nil || cloneKptfile.UpstreamLock == nil {
		t.Fatalf("Expected both Kptfiles to have upstream lock sections")
	}

	// Compare git fields, ignoring commit hashes which may differ
	if getKptfile.Upstream.Git.Repo != cloneKptfile.Upstream.Git.Repo {
		t.Errorf("Upstream repo URLs differ: %q vs %q",
			getKptfile.Upstream.Git.Repo, cloneKptfile.Upstream.Git.Repo)
	}

	if getKptfile.Upstream.Git.Directory != cloneKptfile.Upstream.Git.Directory {
		t.Errorf("Upstream directories differ: %q vs %q",
			getKptfile.Upstream.Git.Directory, cloneKptfile.Upstream.Git.Directory)
	}

	if getKptfile.UpstreamLock.Git.Repo != cloneKptfile.UpstreamLock.Git.Repo {
		t.Errorf("Upstream lock repo URLs differ: %q vs %q",
			getKptfile.UpstreamLock.Git.Repo, cloneKptfile.UpstreamLock.Git.Repo)
	}

	if getKptfile.UpstreamLock.Git.Directory != cloneKptfile.UpstreamLock.Git.Directory {
		t.Errorf("Upstream lock directories differ: %q vs %q",
			getKptfile.UpstreamLock.Git.Directory, cloneKptfile.UpstreamLock.Git.Directory)
	}
}
