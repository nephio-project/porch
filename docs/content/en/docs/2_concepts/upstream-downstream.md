...
weight: 6
description: |
  Understanding upstream and downstream packages: the source and derivation relationship in Porch.
---

## What are Upstream and Downstream Packages?

**Upstream** and **downstream** describe a source-to-derivation relationship between packages in Porch. This relationship is fundamental to package reuse, customization, and lifecycle management.
...
When you create a downstream package from an upstream package, Porch establishes and tracks this relationship. The downstream package maintains a record of its upstream source, including the exact version (Git commit or OCI digest) it was derived from.

This tracking enables:
- **Lineage visibility**: Know where a package came from
...
A package can be both upstream and downstream simultaneously:
...
**Manual workflow**: Users can choose when to upgrade their downstream packages to incorporate upstream changes. Porch merges upstream updates with downstream customizations.

**Automated workflow**: PackageVariant controllers can automatically detect upstream changes and create new downstream package revisions, applying customizations consistently.

**Customization preservation**: Downstream-specific modifications (labels, resources, function configurations) are preserved during upstream updates through intelligent merge strategies.
...
This flexibility allows organizations to consume packages from various sources while maintaining consistent downstream management through Porch.
